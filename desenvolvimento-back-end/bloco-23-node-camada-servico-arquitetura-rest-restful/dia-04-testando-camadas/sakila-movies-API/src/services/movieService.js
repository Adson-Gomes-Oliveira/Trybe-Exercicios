...
const editMovie = async (movieId, movieEdited) => {
  const verify = validations.verifyMovieEdit(movieId, movieEdited);
  if (verify.message) return verify;

  const data = await database.editMovie(movieId, movieEdited);
  return { result: data, code: httpStatus.OK };
}
const deleteMovie = async (movieId) => {
  console.log('Services' + movieId);
  const verify = validations.verifyMovieDelete(movieId);
  if (verify.message) return verify;

  await database.deleteMovie(movieId);
  return { result: { message: 'Sucessfully Deleted' }, code: httpStatus.OK };
}
